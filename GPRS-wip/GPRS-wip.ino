...
const char* constant_string = "GET /gprs/?name=gprs_device&data=1234567890 HTTP/1.1\r\nHost: mbrace.xyz\r\n\r\n";
...
//  gprs_http_send(start_string + middle_string + end_string);
    gprs_http_send(constant_string);
}
...
void gprs_http_send(const char* http_string){
  mySerial.println("AT+CIPOPEN=0,\"TCP\",\"mbrace.xyz\",80");
  wait_for_serial_response();

  mySerial.print("AT+CIPSEND=0,");
  mySerial.println(sizeof(http_string));
  wait_for_serial_response();
 
  mySerial.write(http_string);
  manual_delay(50);
  mySerial.write(0x1a);
  wait_for_serial_response();
...
void manual_delay(int interval){
